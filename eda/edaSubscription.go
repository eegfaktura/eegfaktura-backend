	...
	return []model.Subscriptions{
		{
			...
			Handler:  protcolCrMsgHandler,
		},
		{
			Protocol: model.CR_REQ_PT,
			Handler:  protcolCrReqPtHandler,
		},
		{
			Protocol: model.EC_REQ_ONL,
			Handler:  protocolEcReqOnlHandler,
		},
		{
			Protocol: model.CM_REV_IMP,
			Handler:  protocolCmRevImpHandler,
		},
		//{
...
func protcolCrMsgHandler(msg model.SubscribeMessage) {
	logrus.Printf("Handle Subscriptions: %+v", msg.Protocol)

	historyValue := map[string]interface{}{"meter": msg.Payload.Meter}
	_ = saveHistory(msg.Tenant, string(msg.MessageCode), msg.Payload.ConversationId, "ADMIN", "IN", historyValue)
	return
}

func protcolCrReqPtHandler(msg model.SubscribeMessage) {
	var err error
	logrus.Printf("Handle Subscriptions: %+v", msg.Protocol)
	...
	case model.EBMS_ZP_RES, model.EBMS_ZP_REJ:
		codes, _, _ = extractResponseCodeAndMeteringPoint(&msg.Payload)
	default:
		return
	}

	if err = saveNotification(map[string]interface{}{
	...
	_ = saveHistory(msg.Tenant, string(msg.MessageCode), msg.Payload.ConversationId, "ADMIN", "IN", msg.Payload)
}

func protocolEcReqOnlHandler(msg model.SubscribeMessage) {
	var err error
	logrus.Printf("Handle Subscriptions: %+v", msg.Protocol)
...
func protocolCmRevImpHandler(msg model.SubscribeMessage) {
	var err error
	logrus.Printf("Handle Subscriptions: %+v", msg.Protocol)
	...
	switch msg.MessageCode {
	case model.EBMS_AUFHEBUNG_CCMI, model.EBMS_AUFHEBUNG_CCMS:
...
func saveHistory(tenant, messageCode, conversationId, role, dir string, msg interface{}) error {
	var msgBytes []byte
	var err error
	if msgBytes, err = json.Marshal(msg); err == nil {
		if err = database.SaveEdaHistory(tenant, conversationId, dir, string(msgBytes), messageCode, role); err != nil {
			logrus.Error(err)
			return err
		}
	}
